...
// Dashboard navigation sidebar for Administrator workspace
...
export function AdminSidebar(): React.JSX.Element {
  ...
  const isActive = (href: string) => {
    ...
    return pathname === localizedHref || (href !== '/admin' && pathname.startsWith(localizedHref));
  };
    ...
    <aside className="fixed inset-y-0 left-0 hidden w-64 border-r border-slate-200 bg-white p-4 dark:border-slate-850 dark:bg-slate-950 md:flex md:flex-col justify-between transition-colors duration-300">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-2.5 px-2">
          <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-tr from-indigo-600 to-indigo-700 text-white shadow-md shadow-indigo-600/20">
            <Shield className="h-5 w-5" />
          </div>
          <div>
            <p className="text-xs font-extrabold uppercase tracking-widest text-indigo-600 dark:text-indigo-400">ShopFlow</p>
            <p className="text-sm font-black text-slate-800 dark:text-slate-200">Admin Panel</p>
          </div>
        </div>
              ...
              <Link
                ...
                className={[
                  ...
                  active
                    ...
                    : 'text-slate-650 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-900/60 dark:hover:text-slate-100',
                ].join(' ')}
              >
      ...
      <div className="border-t border-slate-100 dark:border-slate-850 pt-4 space-y-3">
        ...
        <button
          type="button"
          onClick={logout}
          className="w-full flex items-center gap-3 rounded-xl px-3.5 py-2.5 text-sm font-semibold text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/20 transition-all duration-200 focus:outline-none"
        >
